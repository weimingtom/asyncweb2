...
public class HttpServerHandler extends IoHandlerAdapter {
	private static final MyLog _log = MyLog.getLog(HttpServerHandler.class);

    public void sessionOpened(IoSession session) {
    	_log.debug("O %s", session);
...
//        session.setIdleTime(IdleStatus.BOTH_IDLE, 60);
    ...
    public void sessionIdle(IoSession session, IdleStatus status) {
    	_log.debug("I %s / %s", session, status);
        session.close(false);
    }

    public void exceptionCaught(IoSession session, Throwable cause) {
    	_log.error(cause, "E %s / %s", session, cause);
        session.close(true);
    }
}
